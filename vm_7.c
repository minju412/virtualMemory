...
//존재하는 프로세스면 context switch(ptbr을 바꿔줘야함), 존재하지 않으면 fork
{
    ...
    list_for_each(ptr, &processes){
        ...
        if(prc->pid == pid){ //존재하는 프로세스로 switch
            ...
            // prc->pagetable.outer_ptes = malloc(sizeof(struct pte_directory)); //??
...
here:
    if(flag==0){ //존재하지 않는 프로세스로 switch
        printf("forked!\n");
        // struct process* cpy = current;
        // list_add_tail(&current->list, &processes);
        
        // struct process* child = &init; //=current;
        ...
        child.pagetable.outer_ptes[global_pd_index] = malloc(sizeof(struct pte_directory));
        // struct process *child = &child_init;      

        // child.pid = pid;
        printf("%d\n", child.pagetable.outer_ptes[0]->ptes[0].pfn);

        // for(int j=0; j<=global_pd_index; j++){
        //     for(int i=0; i<16; i++){     
        //         child.pagetable.outer_ptes[j]->ptes[i].valid = current->pagetable.outer_ptes[j]->ptes[i].valid;          
        //         child.pagetable.outer_ptes[j]->ptes[i].writable = current->pagetable.outer_ptes[j]->ptes[i].writable;          
             
        //         child.pagetable.outer_ptes[j]->ptes[i].pfn = current->pagetable.outer_ptes[j]->ptes[i].pfn;          
        //     }
        // } //?????????
        ...
        // if(!list_empty(&processes)) printf("ok!\n");
