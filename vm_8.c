...
here:
    if(flag==0){ //존재하지 않는 프로세스로 switch
        ...
        // struct process* child; //이때 current의 포인터를 넣어서
        ...
        // for(int i=0; i<8; i++){
            for(int j=0; j<16; j++){
                ...
                child.pagetable.outer_ptes[0] = malloc(sizeof(struct pte_directory));
                
                struct pte *child_ptes = &child.pagetable.outer_ptes[0]->ptes[j];
                struct pte *current_ptes = &current->pagetable.outer_ptes[0]->ptes[j];

                child_ptes->pfn = current_ptes->pfn;
               

                printf("okkk\n");
                printf("child pfn=, current pfn=%d\n", current_ptes->pfn);
            }
        // }
        ...
        // child->pid = pid; //이때 current의 값이 바뀌어버림!!! -> current의 값들만 복사(이중포문..?)
        printf("child pid=%d\n", child.pid);
