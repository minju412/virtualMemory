...
{ 
	//current의 page table을 construct
    //ptbr을 잘 세팅하면 show는 알아서 출력..??
	...
	// struct pte_directory *pd = malloc(sizeof(struct pte_directory));
	// struct pte *pte = malloc(sizeof(struct pte)*NR_PTES_PER_PAGE);
	...
	// current->pagetable.outer_ptes[pd_index]
    // printf("ptbr=%d\n", *ptbr); //1
    // printf("ptbr=%d\n", &ptbr);
	// printf("ptbr=%d\n", ptbr);
